...
class AttributeModel extends Model
{
	...
	public function getByCid($cid)
	{
		...
		return M()->join("{$join}")->field('attribute_id,attr.name,attr.kind_id')->where("cate_attr.category_cid={$cid}")->select();
	}
	...
	public function getAttr($fields='',$seleAttrId=array())
	{
		if (empty($seleAttrId))
		{
			...
			return $this->field("{$fields}")->where("id not in ({$seleAttrId})")->select();
		}
	}
}