...
CREATE TABLE IF NOT EXISTS `erp`.`supplier` (
  ...
  `brand_id` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '供应商品牌ID',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC))
ENGINE = MyISAM
COMMENT = '供应商表';
...
CREATE TABLE IF NOT EXISTS `erp`.`purchase_order` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `purchase_sn` CHAR(13) NOT NULL DEFAULT '' COMMENT '采购单号',
  ...
  `purchase_status` TINYINT NULL DEFAULT 1 COMMENT '采购单状态 1为采购中，2为已到货，3为部分入库，4为全部入库，5为需退回，6为退回中，7为采购关闭',
  `lock_status` TINYINT NULL DEFAULT 0 COMMENT '锁定状态，0为正常，1为已锁定',
  PRIMARY KEY (`id`),
...
-- -----------------------------------------------------
-- Table `erp`.`admin_role`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `erp`.`admin_role` (
)
ENGINE = InnoDB;
