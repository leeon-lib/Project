...
class lf_checkCode extends lf_controller{
	 ...
	 *
	 */
	public function __construct($width,$height,$bgcolor='#FFFFFF'){
		$this->width = $width;
		$this->height = $height;
	...
	private function writeCode() {
		...
		for ($i=0; $i < strlen($this->code); $i++) { 
			// 计算每个字符在画布中的位置
			$x = ($this->width / $this->codeLength) * $i + 20;
			$y = ($this->height + 20) / 2;
			// 颜色随机
			$fontColor = imagecolorallocate($this->img, 255, mt_rand(0,255), mt_rand(0,255));
			// 写入画布
			imagettftext($this->img, 20, 0, $x, $y, $fontColor, '../../../lprame/font.ttf', $this->code[$i]);
		}
	}
